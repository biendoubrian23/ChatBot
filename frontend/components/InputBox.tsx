...
export default function InputBox({ onSendMessage, isLoading }: InputBoxProps) {
  ...
  useEffect(() => {
    ...
    if (textarea) {
      ...
      // Appliquer la nouvelle hauteur basée sur le contenu
      const newHeight = Math.min(textarea.scrollHeight, 200) // Max 200px (~8 lignes)
  ...
  return (
    <div className="bg-white dark:bg-gray-900 px-4 py-3">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative flex items-end bg-gray-100 dark:bg-gray-800 rounded-3xl px-4 py-2 shadow-sm border border-gray-200 dark:border-gray-700">
          <textarea
            ref={textareaRef}
            value={input}
            ...
            disabled={isLoading}
            rows={1}
            className="flex-1 bg-transparent text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 resize-none outline-none text-sm py-1 overflow-y-auto"
            style={{
              minHeight: '24px',
              maxHeight: '200px',
            }}
        ...
        <p className="text-xs text-gray-400 dark:text-gray-500 mt-2 text-center">
          Propulsé par Mistral AI 🇫🇷 — CoolLibri à votre service
        </p>
      </div>
    </div>
  )
