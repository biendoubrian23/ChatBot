...
export default function OrderStatus({ orderData }: OrderStatusProps) {
  ...
  const getShippingStatus = () => {
    const item = orderData.items[0]; // Première item pour l'exemple
    ...
    <div className="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-green-500 to-blue-600 text-white p-6">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-2xl font-bold">Commande #{orderData.order_id}</h2>
            <p className="text-green-100 mt-1">Commande du {formatDate(orderData.order_date)}</p>
          </div>
          <div className="text-right">
            <div className="text-3xl font-bold">{orderData.total}€</div>
      ...
      <div className="p-6">
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="font-medium text-gray-900 mb-2">{orderData.customer.name}</div>
          <div className="text-sm text-gray-600 space-y-1">
            <div>{orderData.customer.address}</div>
            {orderData.customer.address2 && <div>{orderData.customer.address2}</div>}
            <div>{orderData.customer.zip_code} {orderData.customer.city}</div>
          </div>
        </div>
      </div>
    </div>
